          ...
          {activeTab === 'POINTS' && (
            <div className="space-y-6 animate-fade-in">
              <div className="bg-white border-2 border-emerald-50 p-6 rounded-[32px] flex items-stretch justify-center shadow-sm divide-x divide-gray-100">
                {/* OSTATNIE 3: średnia strzałów + średnia punktów na sesję */}
                <div className="flex flex-col items-center justify-center flex-1 px-2 gap-3">
                  ...
                  <div className="text-center">
                    <p className="text-3xl font-black text-emerald-600 leading-none">{stats.avgPoints3 ? stats.avgPoints3 : '–'}</p>
                  ...
                  <div className="text-center">
                    <p className="text-xl font-black text-[#0a3a2a] leading-none">{stats.avgArrows3 ? stats.avgArrows3 : '–'}</p>
                ...
                {/* MIESIĄC: średnia strzałów + średnia punktów na sesję */}
                <div className="flex flex-col items-center justify-center flex-1 px-2 gap-3">
                  ...
                  <div className="text-center">
                    <p className="text-3xl font-black text-emerald-600 leading-none">{stats.avgPointsMonth ? stats.avgPointsMonth : '–'}</p>
                    <span className="text-[8px] font-bold text-emerald-400 mt-1 block uppercase tracking-tighter">{t('home.quickStats.ringeLabel', { defaultValue: 'Ringe' })}</span>
                  </div>
                  <div className="text-center">
                    <p className="text-xl font-black text-[#0a3a2a] leading-none">{stats.avgArrowsMonth ? stats.avgArrowsMonth : '–'}</p>
                    <span className="text-[8px] font-bold text-gray-400 mt-1 block uppercase tracking-tighter">{t('home.quickStats.pfeileLabel', { defaultValue: 'Pfeile' })}</span>
                  </div>
                </div>
              </div>
                    ...
                    <div className="flex items-end justify-between gap-1 w-full h-32 relative">
                      {weeklyArrows.map((val, i) => {
                          ...
                          return (
                            <div key={i} className="flex flex-col items-center justify-end gap-1 relative flex-1 h-full">
                              <div className="w-full relative flex items-end justify-center h-full">
                                {/* Nad słupkiem: średnia tygodniowa punktów (emerald) + strzały (szary) */}
                                {val > 0 && (
