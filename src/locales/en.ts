export const components = {
  coach: {
    virtualTrainer: "Virtual Coach",
    aiAnalysis: "AI Gemini Analysis",
    previousNote: "Previous note:",
    noPrevious: "No previous notes. This is our first training session together!",
    generateBtn: "Generate Training Report",
    analyzing: "Coach is analyzing data...",
    mockAdvice: "Coach's View: Great job today! Your score is {{score}} points from {{arrows}} arrows (Accuracy: {{accuracy}}%).\n\nI see on the target face that left deflections are smaller. This means you followed my last advice. Focus on aerobic endurance.",
    saveAdvice: "Remember aerobic endurance for the next training session and keep a relaxed grip on the riser."
  },
  export: {
    errorGenerate: "An error occurred while generating the document.",
    title: "Export Results",
    basic: "Basic",
    full: "Full Report",
    pdfTable: "PDF Table",
    pdfCsv: "PDF + CSV",
    errorAuth: "Error: Cannot export, user not logged in.",
    errorEmpty: "No arrows on target face. Add scores to export report.",
    unlockPro: "Unlock Full PRO Report",
    pdfScorecard: "Scorecard - Training Session",
    pdfDate: "Date",
    pdfRound: "ROUND",
    pdfArrows: "Arrows",
    pdfSum: "Sum",
    pdfTotal: "Total",
    pdfGrandTotal: "GRAND TOTAL",
    pdfAverage: "AVERAGE",
    pdfArrowsCount: "ARROWS",
    pdfResult: "SCORE",
    pdfCoachNotes: "Coach Notes / Equipment Adjustments:",
    pdfCoachPlaceholder: "(Space for physical coach notes, sight marks, clicker settings, etc.)",
    csvHeader: "End,Arrow_1,Arrow_2,Arrow_3,Arrow_4,Arrow_5,Arrow_6,End_Total"
  },
  scoring: {
    round: "Round",
    series: "End",
    total: "Total",
    zoom: "Zoom target view",
    trendTitle: "Session Trend",
    trendHint: "Click bar to highlight"
  },
  timer: {
    ready: "Ready",
    prep: "Prep",
    shoot: "Shoot",
    finished: "Finished",
    collapse: "Hide",
    round: "Round",
    end: "End",
    seconds: "seconds",
    stop: "STOP / RESET"
  },
  notifications: {
    defaultName: "Archer",
    newYearTitle: "Happy New Year, {{name}}! 🎆",
    newYearMsg: "We officially begin the {{year}} season. It's a great time for new personal bests!",
    classChanged: "This year you're moving to a new age category! I've updated your default distances and target face sizes according to federation rules.",
    classSame: "This year you remain in your current age category. Your distances and tournament target faces remain unchanged.",
    newYearWish: "Wishing you straight arrows!",
    newYearClose: "Thanks, Coach!",
    birthdayTitle: "Happy Birthday, {{name}}! 🎂",
    birthdayMsg: "On your birthday, I wish you a steady hand, calm breath, and only arrows in the very center of the target face (preferably all Xs)!",
    birthdayInfo: "Remember: in archery (WA/DSB), age classes don't change on your birthday, but always by birth year (January 1st). Your current distances stay the same for now!",
    birthdayClose: "Got it, time to celebrate!"
  },
  tournamentInput: {
    cardTitle: "SCORECARD",
    modeDetailed: "Detailed",
    modeSummary: "Summary",
    totalScore: "Total Score",
    points: "Points",
    finishedMsg: "Done! Click any arrow to correct it.",
    editMode: "Edit mode",
    hideKeyboard: "Minimize keyboard",
    fixScores: "Fix scores",
    showKeyboard: "Show keyboard (Typing...)",
    aiPlaceholder: "Note for AI (wind, stress, new bowstring)...",
    saving: "Saving...",
    saveBtn: "Save Tournament Result",
    durchgang: "Round",
    passes: "ends",
    avgPerArrow: "Avg / arrow",
    avgDurchgang: "Avg round",
    avgTotal: "Avg match",
    practiceArrowsLabel: "Practice arrows",
    practiceNote: "* Practice arrows are not included in the average",
    fromDetailed: "Data from detailed mode. Go back to Detailed to edit ends.",
    fallbackMode: "Fallback mode",
    fallbackDesc: "Use when you lost your scorecard. Enter round totals only.",
    durchgang1: "Round 1",
    durchgang2: "Round 2"
  },
  weather: {
    windUnit: "km/h",
    error: "Weather fetch error:"
  },
  rules: {
    bow_recurve: "Recurve",
    bow_compound: "Compound",
    bow_barebow: "Barebow",
    bow_trad: "Traditional",
    discipline_blowgun: "Blowgun",
    season_indoor: "Indoor",
    season_outdoor: "Outdoor"
  },
  // DSB age classes — display labels (canonical DB value stays German,
  // see formatAgeCategory in utils/privateProfile.ts)
  ageCategory: {
    schuelerC: "Kids (U11)",
    schuelerB: "Youth (11–12)",
    schuelerA: "Youth (13–14)",
    jugend: "Junior (15–17)",
    junioren: "Junior (18–20)",
    damen: "Women",
    herren: "Men",
    master: "Master (50–65)",
    senioren: "Senior (66+)",
    genderM: "M",
    genderF: "W",
  },
  common: {
    comingSoon: "Coming soon",
    noClubsFound: "No matching clubs found",
    clubNameWarning: "This name is not in the database. Please enter the club name as accurately as possible so it can be added correctly and prevent other members from registering the same club under a different name.",
    firstClubInCity: "You are the first person from this city/club — it will be added to the database. Please enter the club name as accurately as possible so other members don't add the same club under a different name.",
    clubFoundInDb: "Club found in database ✓",
    noClubManual: "Club not found? Enter manually",
    clubCityPlaceholder: "Club city",
    selectCityFirst: "Select a city first",
    clubNamePlaceholder: "Club name",
    arrows: "arrows",
    day: "Day",
    confirm: "Confirm",
    saving: "Saving...",
    edit: "Edit",
    cancel: "Cancel",
    save: "Save",
    month: "Mo.",
    year: "Year",
    hour: "Hr.",
    optional: "(Optional)",
    export: {
      errorGenerate: "An error occurred while generating the document."
    }
  },
  appMove: {
    title: "GROT-X has a new address",
    body: "From now on the app lives at {{host}}. Your trainings, journal and settings are safe in your account.",
    steps: "Open the new address, log in once and add the icon to your home screen. You can delete the old icon.",
    sending: "Sending saves from your phone…",
    offline: "Connect to the internet first — saves on your phone still need to reach your account.",
    open: "Open new address",
    later: "Later"
  },
  appUpdate: {
    available: "New version available",
    reload: "Refresh",
    later: "Later"
  },
  offline: {
    noNetwork: "No signal — entries will wait on your phone",
    queued: "No signal — entries waiting: {{count}}. They will send automatically",
    sending: "Sending entries from your phone ({{count}})…",
    sent: "All sent",
    failed: "Server rejected an entry saved offline"
  },
  messages: {
    headerSubtitle: "{{max}} msg max · 1 msg / 10 min",
    loading: "Loading…",
    empty: "Start the conversation",
    cooldown: "Next message in",
    placeholder: "Write a message…",
    min: "min",
    sek: "sec",
    errorPermission: "No permission to send this message.",
    errorConnection: "Connection error. Please try again."
  },
  coachLog: {
    title: "Coach Diary",
    subtitle: "Visible to all coaches",
    addBtn: "Entry",
    typeObservation: "Observation",
    typeTip: "Tip",
    typeGoal: "Focus",
    typeFlag: "Important",
    placeholder: "Write an entry for the other coaches…",
    cancel: "Cancel",
    save: "Save",
    saving: "…",
    loading: "Loading…",
    empty: "No entries yet",
    emptyCoachHint: "Leave the first entry for the coaching team",
    deleteTitle: "Delete entry?",
    deleteDesc: "This action cannot be undone.",
    delete: "Delete",
    defaultCoachName: "Coach",
    hint: "Note here what you last worked on with the student and what to focus on in the next training sessions.",
    focusGoalLabel: "How many sessions?",
    focusHint: "Manage the session count and ending the focus early from the focus panel — tap the “Focus” bar in the student profile."
  }
};

export const views = {
  auth: {
    title: "GROT-X",
    subtitle: "Your Virtual Archery Coach",
    email: "Email",
    password: "Password",
    loginBtn: "Login",
    registerBtn: "Create Account",
    tabLogin: "Log in",
    tabRegister: "Create account",
    switchLogin: "Login",
    switchRegister: "Sign up",
    noAccount: "Don't have an account?",
    haveAccount: "Already have an account?",
    googleAuth: "Quick Google Login",
    or: "OR",
    errorInvalid: "Invalid email or password.",
    errorGeneral: "Authentication error. Please try again.",
    errorEmailInUse: "An account with this email already exists. Log in or reset your password.",
    errorWeakPassword: "The password is too weak.",
    errorPasswordRules: "The password needs: {{list}}.",
    pwMinLength: "at least {{count}} characters",
    pwUpper: "an uppercase letter",
    pwLower: "a lowercase letter",
    pwNumber: "a number",
    pwSpecial: "a special character (e.g. !?#)",
    errorInvalidEmail: "Invalid email address.",
    errorTooMany: "Too many attempts. Please wait a moment and try again.",
    errorNetwork: "No internet connection. Please try again.",
    googleError: "Google login failed. Try opening the app in a new window.",
    forgotPassword: "Forgot password?",
    resetTitle: "Account Recovery",
    resetBtn: "Send reset link",
    backToLogin: "Back to login",
    resetSuccess: "Password reset link has been sent to your email! 📧",
    privacyAccept: "I accept the",
    privacyPolicy: "Privacy Policy",
    privacyRequired: "Please accept the Privacy Policy to create an account.",
    privacyGoogleNote: "By continuing with Google you accept the Privacy Policy.",
    guestBtn: "Try without an account",
    guestNote: "Guest data is automatically deleted after 24 hours."
  },
  guest: {
    banner: "Guest mode — data expires in {{time}}.",
    bannerCta: "Create account",
    blockedFeature: "Battles and rankings require a full account.",
    upgradeTitle: "Keep your data",
    upgradeDesc: "Create an account and your guest trainings and settings will stay with you permanently.",
    upgradeBtn: "Create account",
    upgradeGoogle: "Continue with Google",
    emailInUse: "This e-mail already has an account. Sign out of guest mode and log into it.",
    weakPassword: "Password must be at least 6 characters."
  },
  parentalConsent: {
    title: "Parental consent required",
    intro: "You are under {{age}}, so you may only use the app with the consent of a parent or legal guardian. Please have them confirm below.",
    guardianEmail: "Parent / guardian email",
    guardianEmailPlaceholder: "guardian's email",
    affirm: "As a parent or legal guardian, I consent to this child using the app and I accept the",
    privacyPolicy: "Privacy Policy",
    confirmBtn: "Confirm consent",
    logout: "Log out",
    note: "We store the consent as a record (Art. 8 GDPR). It can be withdrawn by deleting the account in Settings.",
    errorEmail: "Please enter a valid guardian email address.",
    errorAffirm: "Please tick the consent box to continue.",
    errorSave: "Could not save consent. Please try again."
  },
  nav: {
    home: "Home",
    calendar: "Schedule",
    stats: "Stats",
    settings: "Settings",
    tagebuch: "Journal",
    coach: "Trainer",
    training: "Training",
    inProgress: "LIVE"
  },
  tagebuch: {
    title: "Journal",
    notePlaceholder: "Add a note…",
    onlyYou: "Only you",
    visibleCoach: "Visible to your coach",
    myNote: "My note",
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    deleteNoteTitle: "Delete note?",
    saveError: "Couldn't save the note. Try again.",
    mic: "Microphone",
    recording: "Recording…",
    topic: "Topic",
    allTopics: "All topics",
    next: "Up next",
    fromCoach: "from your coach",
    filterAll: "All",
    filterCoach: "Coach",
    filterMine: "Mine",
    today: "Today",
    yesterday: "Yesterday",
    tomorrow: "Tomorrow",
    neu: "New",
    training: "Training",
    techTraining: "Technique training",
    rings: "points",
    noteShort: "Note",
    coachNote: "Coach note",
    coachEntry: "Coach entry",
    inviteTitle: "Training with a coach?",
    inviteDesc: "Connect, and your coach's notes will show up right here.",
    inviteBtn: "Connect a coach",
    loading: "Loading…",
    loadMore: "Load older",
    emptyTitle: "Your journal is empty",
    emptyDesc: "Training sessions and notes show up here automatically.",
    emptyFilter: "No entries for this filter",
    thisWeek: "This week",
    lastWeek: "Last week",
    showWeek: "Show week",
    weekTrainings: "{{count}} trainings",
    weekTrainings_one: "{{count}} training",
    avgShort: "avg {{avg}}",
    avgUnit: "avg",
    vsLast: "vs last",
    record: "Record",
    bestOfLast: "Best of {{count}}",
    focusLabel: "Your focus",
    focusFromCoach: "from coach",
    focusLastDone: "Your last completed focus",
    focusMilestoneDone: "Focus completed",
    focusMilestoneEnded: "Focus ended",
    coachSeen: "Your coach has read your note",
    focusEmpty: "Set a focus: what are you working on?",
    focusCount: "{{hit}} of {{goal}} trainings",
    focusCountOne: "{{hit}} of 1 training",
    focusDone: "Done! {{count}} of {{goal}}",
    focusPickNew: "Pick a new focus",
    focusDotsOff: "Off",
    focusDotsHint: "Each technique training on this topic fills a dot. Scored trainings count via the switch when you save.",
    focusSince: "since {{date}}",
    focusNewPlaceholder: "Briefly describe what you are working on…",
    focusGoalWarning: "You already have {{count}} trainings on this topic — the focus will count as done right away.",
    focusEnd: "End focus",
    focusEndConfirm: "End your focus “{{topic}}”?",
    focusEndConfirmHint: "Only the focus card goes away. Your trainings and notes stay in your journal.",
    focusEndYes: "Yes, end it",
    focusBack: "Back",
    focusCongrats: "Congratulations!",
    focusCongratsText: "You've completed the full {{goal}}-step training on “{{topic}}”.",
    focusComplete: "Complete focus",
    focusSaveError: "Couldn't save your focus. Try again.",
    focusEditBecomesOwn: "After saving it becomes your own focus — start date and progress stay.",
    focusChip: "Focus",
    newNote: "New note",
    coachChat: "Chat with coach",
    yourCoaches: "Your coaches",
    ends: "Ends",
    showStats: "Stats",
    filters: "Filters",
    closeFilters: "Close filters",
    inviteHide: "Hide"
  },
  home: {
    archerRanking: "Archer Ranking",
    battleSubtitle: "Challenges · Stats · Ranking",
    activeChallenges: "Active challenges",
    battleStats: "Battle stats",
    active: "active",
    yourStudentId: "Your unique student ID:",
    copiedId: "ID copied!",
    motivational: "Motivational",
    xpAndHandicap: "XP Ranks + Archery Handicap",
    yourRank: "Your rank",
    yours: "Yours",
    handicapTitle: "Archery Handicap",
    commander: "Command Centre",
    coachTeaser: "Coach mode · 0/{{count}} students",
    nextGoal: "Next Goal",
    calendar: "Schedule",
    today: "Today!",
    tomorrow: "Tomorrow!",
    inDays: "In {{count}} days",
    noLocation: "No location",
    statsTitle: "Shot Statistics",
    month: "Month",
    day: "Today",
    avg14d: "Avg 14 days",
    yearly: "Year",
    refresh: "Refresh stats",
    refreshing: "Refreshing…",
    pfeilzähler: "Arrow Counter",
    techAdd: "Add Technical",
    lastResult: "Last Result",
    arenaLive: "Arena Live",
    arenaSub: "Battles & Rankings",
    arenaWorld: "Club & World",
    arenaMultiTitle: "Multiplayer Arena",
    arenaMultiSub: "Join a club game",
    arenaJoinCode: "Or enter Arena PIN",
    arenaCodePlaceholder: "e.g. AB12CD",
    arenaJoinBtn: "JOIN",
    arenaCreateBtn: "Create New Arena",
    arenaClubLabel: "Club",
    arenaNoClubGames: "No active club games",
    arenaInvalidCode: "No Arena found with this code.",
    arenaFinished: "This Arena has already ended.",
    connectTitle: "Connection",
    connectSub: "Choose connection method",
    yourId: "Your ID:",
    aiCoach: "AI Coach Analysis",
    aiPlaceholder: "Complete a session so AI can evaluate you!",
    qrTitle: "Your QR ID",
    close: "Close",
    join: "Join",
    challenge: "Challenge",
    private: "Private",
    quickStats: {
      title: "Quick Stats",
      tabArrows: "Arrows",
      tabPoints: "Points & Accuracy",
      trendArrows: "Trend: Arrows Shot (52 weeks)",
      trendPoints: "Trend: Point Average (52 weeks)",
      avgPoints: "Points per arrow",
      avgMonthLabel: "Monthly average",
      avgLast3Label: "Average last 3",
      ringeLabel: "Rings",
      pfeileLabel: "Arrows",
      allDistShort: "All",
      unlockTrend: "Unlock Yearly Trend",
      buyPro: "Unlock GROT-X PRO"
    },
    trendModal: {
      subtitle: "Recent sessions",
      title: "Score curve",
      typeTraining: "Training",
      typeTournament: "Tournament",
      typeArena: "Arena",
      noData: "No data",
      loadMore: "Show more",
      unlockMore: "Unlock PRO to see more",
    },
    arenaDeleted: "Arena has been deleted.",
    arenaDeleteError: "Error while deleting.",
    rankTooFar: "Rank too far apart (You: {{myLevel}}, Host: {{hostLevel}}). Max ±1 level.",
    done: "Completed",
    battleHistory: "Full battle history",
    yourRanking: "Your ranking",
    arenaDeleteConfirm: "Are you sure you want to delete this Arena? It will disappear for all club members.",
    handicapDesc: "Normalized to Olympic reference (70m / 122cm). The lower — the better.",
    handicapEmpty: "Your handicap will be calculated automatically after the first saved training session.",
    xpHowTitle: "How to earn XP?",
    xpPerSession: "For each training session:",
    xpExample: "Example",
    xpExampleText: "60 arrows, score 510 pts → average 8.5",
    xpHighRankNote: "Ranks 7–10 (RED / GOLD) additionally require a minimum average from the last 10 sessions.",
    xpFormula: "XP = arrows × 1 + average × 10",
    noActiveBattles: "No active",
    handicap: {
      elite:           "Elite",
      eliteDesc:       "National team level. Arrows almost exclusively in 9–10.",
      advanced:        "Advanced",
      advancedDesc:    "Consistently in red and yellow. Solid club competitor.",
      competitor:      "Competitor",
      competitorDesc:  "High-level amateur. Results of 7–9 dominate.",
      intermediate:    "Intermediate",
      intermediateDesc:"Solid technique. Scores of 6–8, occasional fives.",
      amateur:         "Amateur",
      amateurDesc:     "Developing your technique. Occasional threes and misses.",
      beginner:        "Beginner",
      beginnerDesc:    "Learning the basics. Focus on repeatability."
    },
    trialWarningTitle: "Free PRO expires in {{days}} {{unit}}! 🚀",
    trialWarningDay: "day",
    trialWarningDays: "days",
    trialWarningContent: "After this time your account will return to FREE version. If you enjoy GROT-X, keep access to all features by upgrading to PRO.",
    trialWelcomeTitle: "You have received 30 days of GROT-X Pro for free! 🎉",
    trialWelcomeContent: "Explore all PRO features without any restrictions. Enjoy your free trial!",
    proGiftTitle: "You've been gifted GROT-X PRO! 🎁",
    proGiftContent: "Your PRO access is now active until {{date}}. Thank you for your support!"
  },
  setup: {
    title: "Setup",
    challengeReceived: "Challenge Received!",
    arenaInvite: "You've been invited to a battle at",
    acceptBtn: "Accept & Compete",
    selectDistance: "Select Distance",
    noDistances: "No distances — enable them in Settings",
    setupSwitchFailed: "Couldn't switch setup. Please try again.",
    sightTitle: "Sight Settings",
    sightPro: "Sight PRO",
    sightExt: "Extension",
    sightHeight: "Elevation (up/down)",
    sightSide: "Windage (L/R)",
    targetTitle: "Target Face",
    selected: "Selected:",
    techBtn: "Technical Training",
    battleBtn: "Multiplayer",
    continueBtn: "Continue previous",
    warningTitle: "Clear scores?",
    warningConfirm: "Clear and start",
    warningCancel: "Cancel",
    change: "Change",
    distSettings: "Distance settings",
    allDistances: "All distances in Settings",
    editorTitle: "Sight",
    editorSave: "Save",
    editorExt: "Extension",
    editorHeight: "Elevation",
    editorSide: "Windage"
  },
  announcements: {
    title: "Messages & Announcements",
    subtitle: "Community messages",
    important: "Important",
    info: "Info",
    welcomeTitle: "Welcome to GROT-X!",
    welcomeMsg: "We're glad to have you with us. The app is in beta — let us know if you have any suggestions!",
    empty: "No notifications",
    emptyDesc: "Messages from your coach and system announcements will appear here.",
    allDeleted: "You have deleted all notifications.",
    deleteTitle: "Delete notification?",
    deleteBtn: "Delete",
    pageTitle: "Notifications",
    badgeCoach: "Coach",
    badgeSystem: "System",
    deleteConfirmDesc: "This notification will be permanently removed from your list.",
    justNow: "just now",
    minutesAgo: "{{count}} min ago",
    hoursAgo: "{{count}} h ago",
    daysAgo: "{{count}} days ago",
    newMessage: "New message",
    newAnnouncement: "New announcement",
    newCoachPlan: "New plan from coach",
    newCoachNote: "New training note",
    newCoachLog: "New diary entry",
    newCoachRequest: "New trainer application"
  },
  battleHistory: {
    title: "BATTLE",
    titleGold: "SCORES",
    subtitle: "Your Recent Challenges",
    empty: "No data to display.",
    modeClub: "Club",
    modeWorld: "World",
    modePrivate: "Private",
    host: "Host:",
    unknown: "Unknown",
    yourScore: "Your Score",
    modalRanking: "ARENA RANKING",
    modalClub: "Club Challenge",
    modalWorld: "World Arena",
    modalPrivate: "Duel",
    me: "You",
    closeBtn: "Close Leaderboard"
  },
  worldQueue: {
    network: "── GROT-X BATTLE NETWORK ──",
    title: "MATCHMAKING",
    protocol: "WORLD PROTOCOL v2.4",
    combatRating: "YOUR RANK",
    searching: "Searching for opponent",
    expanding: "Expanding search radius",
    expandingMore: "Broadening search range",
    scanRadius: "SCAN RADIUS ±{{n}}",
    timeRemaining: "TIME REMAINING",
    noMatchFound: "⚠ NO MATCH FOUND",
    abort: "◄ ABORT",
    abortSearch: "◄ ABORT SEARCH",
    timeout: "No opponent found. Try again.",
  },
  worldLeaderboard: {
    title: "WORLD Ranking",
    loading: "Loading ranking…",
    empty: "No players in this category",
    emptyDesc: "Be the first! Play a WORLD Battle.",
    footer: "Ranking refreshed every 10 min · +5 XP for playing · +10 XP for winning",
    wins: "W",
    losses: "L",
    homeButton: "WORLD Ranking",
  },
  battleLobby: {
    title: "Arena Lobby",
    arenaCode: "ARENA CODE",
    scan: "Scan",
    local: "Local",
    club: "Club",
    world: "World",
    comingSoon: "Soon",
    comingSoonFull: "Coming soon",
    unaffiliated: "Unaffiliated",
    host: "Host",
    startChallenge: "Start Challenge",
    waitingForHost: "Waiting for Host...",
    waitingForPlayers: "Waiting for players",
    cancel: "Cancel",
    archer: "Archer",
    create: "CREATE GAME",
    privacyTitle: "Privacy (Arena)",
    privacyFullName: "Show full name",
    privacyFullNameDesc: "If disabled, we'll show only the initial.",
    privacyNickname: "Show Nickname",
    privacyNicknameDesc: "Appends 'Nick' to your name.",
    privacyClub: "Show my Club",
    privacyRegion: "Show my Region",
    privacyAutoSave: "Changes are saved automatically.",
    inviteSent: "Invitation sent",
    awaitingAcceptance: "Awaiting acceptance...",
    createFirst: "Before adding a player, create a game (button below)!",
    cancelGame: "Cancel game",
    cancelGameTitle: "Cancel game?",
    cancelGameDesc: "The game will be deleted. This action cannot be undone.",
    no: "No",
    yesCancel: "Yes, cancel",
    removePlayer: "Remove player",
    removePlayerTitle: "Remove player?",
    yesRemove: "Yes, remove",
    addBtn: "Add participant",
    cameraError: "No camera access",
    cameraErrorDesc: "Enter participant UID manually:",
    uidPlaceholder: "Participant UID...",
    guests: "guests",
    guestNamePlaceholder: "Player name...",
    worldCooldownMsg: "Free players can join WORLD games once every 2 days.",
    remaining: "Remaining",
    hours: "hrs.",
    phonelessPlayers: "Players without phone",
    addGuestDesc: "Add players without GROT-X account",
    cooldownTitle: "Cooldown Active",
    modeInfoLocal: "Game for up to 4 players in the same place. Invite others by scanning their QR code. You can also add guests — players without a phone.",
    modeInfoClub: "Game for up to 12 club members. Players can join within 3 hours of the game starting.",
    modeInfoWorld: "Automatic matchmaking with an archer of similar rank from around the world. Wait time up to 5 minutes.",
  },
  calendar: {
    title: "Schedule",
    addEvent: "New Event",
    editEvent: "Edit Event",
    seeResult: "SEE RESULT",
    tabTournament: "TOURNAMENT",
    tabOther: "OTHER",
    tabTrainer: "COACH",
    trainerStudents: "Students",
    trainerPlanHint: "Selected students will see this event in their Plan tab.",
    trainerPlanHintNone: "Nobody will see this event — only you.",
    trainerAllStudents: "All",
    trainerNobody: "Nobody",
    trainerSelectedCount: "{{n}} selected",
    formTrainerName: "Session name",
    collapseTrainer: "Collapse coach list",
    noEvents: "No upcoming events",
    upcomingTournaments: "Upcoming Tournaments",
    trainingsAndOthers: "Training Sessions & Other",
    history: "Competition History",
    nextStart: "Next Start",
    wholeDay: "All day",
    collapseTournaments: "Collapse tournaments",
    collapseOthers: "Collapse list",
    collapseHistory: "Collapse history",
    historyLocked: "History limited to 60 days.",
    proHistoryInfo: "Activate GROT-X PRO to keep archives up to 2 years.",
    proHistoryActive: "Showing history (PRO limit: 2 years).",
    autoArchive: "Older events are archived automatically.",
    formTourName: "Tournament name",
    formOtherName: "Event name",
    formDistLabel: "Tournament Distance",
    formCity: "City / Address",
    formNotes: "Notes...",
    formSave: "Save Event",
    formSaving: "Saving...",
    modalSight: "Sight",
    modalSightExt: "Extension",
    modalSightGD: "Elevation",
    modalSightLP: "Windage",
    modalEnterScore: "Enter Score",
    modalActiveFrom: "Active from",
    modalDateTime: "Date & Time",
    dateErrorRequired: "Fill in day, month and year.",
    dateErrorInvalid: "Invalid date — this date does not exist.",
    dateErrorYear: "Year must be between 2000 and 2100.",
    dateErrorMonth: "Month must be between 1 and 12.",
    dateErrorDay: "Day must be between 1 and 31.",
    modalPlace: "Venue",
    modalNotes: "Notes",
    modalEdit: "Edit",
    modalDelete: "Delete",
    confirmDelete: "Are you sure you want to delete this event from the schedule?",
    todoSection: "To-Do",
    todoToggle: "No date — to-do",
    todoDone: "Mark as done",
    archiveTournaments: "Archive: Tournaments",
    archiveCalendar: "Archive: Calendar",
    archiveTrainerReceived: "Archive: Trainer Plans",
    archiveTrainerSent: "Archive: Sent Plans",
    archiveTodo: "Archive: Tasks",
    archiveLoadMore: "Load 5 more",
    archiveEmpty: "No entries",
    archivePts: "pts",
    archiveNoScore: "No result",
    archivePastTournament: "Past Tournament",
    formPastTournaments: "Your previous tournaments",
    legend: "Legend",
    legendTournament: "Tournament",
    legendOther: "Other",
    legendTrainerReceived: "My appointments from coach",
    legendTrainerSent: "Appointments as coach",
    days: ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"]
  },
  profile: {
    loading: "Loading profile...",
    title: "Grot-X Profile",
    subtitle: "AI Analysis Database",
    biometryTitle: "Biometrics & Experience",
    age: "Age (Years)",
    height: "Height (cm)",
    handednessRH: "Right Handed (RH)",
    handednessLH: "Left Handed (LH)",
    experience: "Experience (Years)",
    equipmentTitle: "Equipment Parameters",
    drawWeight: "Draw Weight (LBS)",
    drawLength: "Draw Length (Inches)",
    coachName: "Robin Grotski",
    coachVerified: "Verified AI",
    coachDesc: "Your Personal AI Coach. I analyze this data to tailor your tournament tactics and advise you on technical adjustments.",
    saveBtn: "Save Grot-X Profile",
    saving: "Saving...",
    placeholderAge: "e.g. 28",
    placeholderHeight: "e.g. 180",
    placeholderDraw: "e.g. 28.5"
  },
  scoringView: {
    partialTitle: "Incomplete training",
    partialDesc: "You shot {{shot}} of {{planned}} ends. Should this score count like a full training?",
    partialMark: "Mark as incomplete",
    partialMarkHint: "Arrows and average per arrow count; the total stays out of the curve and per-training averages",
    partialDiscard: "Exit without saving",
    partialDiscardHint: "This training's results will be lost.",
    trainingJournal: "Training Journal",
    confirmAbort: "Yes, abort",
    previewTitle: "Shot Grouping",
    allEnds: "ALL",
    endAbbr: "E",
    keyboardTab: "Keyboard",
    targetTab: "Target",
    round: "Round",
    totalSum: "Total Score",
    avg: "AVG",
    arrows: "ARROWS",
    result: "SCORE",
    saveTraining: "Save session",
    saving: "Saving...",
    abort: "Abort",
    abortConfirm: "Abort training session?",
    arenaClub: "Club Arena",
    arenaWorld: "World Arena",
    arenaLive: "Live Arena",
    ranking: "Leaderboard",
    rankingAfterR1: "Rank after R1",
    rankingAlert: "Leaderboard available after Round 1 (6 ends).",
    liveRanking: "Live Ranking",
    backToTarget: "Back to target face",
    menuHome: "Home",
    menuCalendar: "Schedule",
    menuSettings: "Settings",
    errorNoArrows: "No arrows on target. Add results.",
    notePlaceholder: "Describe your session: wind, fatigue, equipment changes... AI Coach will take this into account!",
    shareWithCoach: "Share note with coach",
    focusToggle: "Focus: {{topic}}",
    focusToggleHint: "Counts toward your focus progress",
    showStats: "Show statistics & AI",
    abortConfirmTitle: "Abort training?",
    abortConfirmDesc: "All unsaved results will be permanently lost.",
    cancelAbort: "Back to shooting",
    guestsBtn: "Guests",
    guestMode: "Guest mode",
    guestTotal: "Total",
    guestEnterEnd: "Enter end arrows",
    guestConfirmEnd: "Confirm end",
    pts: "pts",
    collapse: "Collapse",
    thisSeries: "This series",
    backspace: "Undo"
  },
  settings: {
    copied: "Copied!",
    mainTitle: "Settings & Profile",
    tabProfile: "PROFILE",
    tabSight: "DISTANCES",
    tabArrows: "ARROWS",
    tabBow: "BOW",
    tabLanguage: "LANGUAGE",
    tabEquipment: "EQUIPMENT",
    equipment: {
      setups: "Setups",
      addSetup: "Add",
      proForMore: "PRO = 4 setups",
      setupName: "Setup name",
      defaultSetupName: "My setup",
      newSetupName: "Setup {{n}}",
      removeSetup: "Delete setup",
      confirmDelete: "Delete this setup?",
      confirmDeleteDesc: "All equipment entered for this setup will be lost. Sessions that used it keep their equipment class and stay in your statistics.",
      discipline: "Discipline",
      color: "Setup color",
      colorHint: "The color marks this setup's distances under DISTANCES and at training start.",
      colorTaken: "Another setup already uses this color",
      colors: {
        red: "Red", orange: "Orange", yellow: "Yellow", green: "Green",
        teal: "Teal", blue: "Blue", purple: "Purple", pink: "Pink"
      },
      note: "Note",
      notePh: "Short note for this tab",
      blowgun: {
        model: "Blowgun model",
        modelPh: "e.g. Fukiya 120 cm"
      },
      subtab: {
        archer: "ARCHER",
        blowgun: "BLOWGUN",
        bow: "BOW",
        string: "STRING",
        arrows: "ARROWS",
        sight: "SIGHT",
        stabilization: "STABILISERS"
      },
      info: {
        aria: "About this field",
        ifYouChange: "What happens if you change it",
        close: "Got it",
        discipline: {
          what: "The equipment class you shoot — it drives the rulebook, the recommended distances and what your scores are compared against.",
          change: "Every new session gets the new class. Old scores stay with the old one, so records and averages will not mix — but your progress chart splits into two series."
        },
        "archer.drawLength": {
          what: "Your draw length in inches, measured from the arrow nock groove to the deepest point of the grip, plus 1¾ inches.",
          change: "The value itself does not recalculate scores — it drives arrow length and bow weight choices. Change it when your stance or form has actually changed."
        },
        "bow.lbs": {
          what: "Limb draw weight in pounds at YOUR draw length — not the number printed on the limbs.",
          change: "More weight means a flatter trajectory and different sight marks. After a limb change, review the marks at every distance."
        },
        "bow.riser": {
          what: "The riser model — the centre section the limbs, sight and stabilisers bolt onto.",
          change: "A new riser shifts the sight's zero. Marks from the previous bow stop being trustworthy."
        },
        "bow.limbs": {
          what: "The limb model — limbs store the draw energy and set both the weight and the character of the shot.",
          change: "New limbs change arrow speed and weight, so sight marks have to be shot in again from scratch."
        },
        "string.model": {
          what: "String material and model. Different materials creep differently and damp vibration differently.",
          change: "A new string almost always changes brace height and nocking point — check both before your first end."
        },
        "string.strands": {
          what: "Number of strands in the string. It sets the string's thickness and therefore how the nock fits.",
          change: "Fewer strands means a faster but less durable string. Check the nock still grips the string with the right resistance."
        },
        "string.nockingPoint": {
          what: "Where the arrow sits relative to square with the riser, usually given in millimetres above.",
          change: "Even a few millimetres change vertical grouping. Move it one step at a time and shoot a check end."
        },
        "arrows.model": {
          what: "Shaft model and type. Material and diameter drive wind drift, speed and how well the shaft survives hits.",
          change: "A different arrow means different mass and stiffness, so sight marks and grouping shift from the very first end."
        },
        "arrows.spine": {
          what: "Shaft stiffness — the lower the number, the stiffer the arrow. It must match your bow weight and arrow length.",
          change: "The wrong spine leaves the bow crooked. It shows up as lateral drift that no sight adjustment can fix."
        },
        "arrows.length": {
          what: "Shaft length, usually measured from the nock groove to the end of the shaft, point excluded.",
          change: "A shorter arrow behaves stiffer. Cutting changes the spine you selected and cannot be undone."
        },
        "sight.model": {
          what: "Sight model. You record it so that after a swap you know which equipment the saved marks belong to.",
          change: "Sight marks in the DISTANCES tab were shot in for one specific sight and extension. After a swap treat them as a starting point, not a result."
        },
        "stabilization.description": {
          what: "Stabiliser setup: lengths, weights, side rods. It sets the bow's balance and how it behaves after the shot.",
          change: "Changing the balance changes your hold and aiming rhythm. Write down the setup that works before you start experimenting."
        }
      },
      string: {
        model: "String model",
        modelPh: "e.g. BCY 8125G",
        strands: "Strand count",
        strandsPh: "e.g. 18",
        nockingPoint: "Nocking point",
        nockingPointPh: 'e.g. +6 mm'
      }
    },
    themeTitle: "Appearance",
    themeLight: "Light",
    themeDark: "Dark",
    themeSystem: "System",
    tabPro: "PRO",
    tabCoach: "COACH",
    tabTournament: "EVENTS",
    tabShare: "SHARE",
    shareTitle: "Share GROT-X",
    shareSubtitle: "Scan the code with your phone's camera — it opens the app in your browser and lets you install it.",
    saveAll: "Save & Update",
    saving: "Saving...",
    successSave: "Saved successfully.",
    logoutConfirm: "Are you sure you want to log out?",
    privacySection: "Privacy (Rankings)",
    privacyData: "Privacy & data (GDPR)",
    privacyPolicyLink: "Privacy Policy",
    impressumLink: "Impressum",
    exportData: "Export my data (JSON)",
    exportRunning: "Preparing export...",
    exportDone: "Data exported — check your downloads.",
    exportError: "Export failed. Please try again.",
    deleteAccount: "Delete account",
    deleteWarning: "This permanently deletes your account, profile and all training data. This action cannot be undone.",
    deleteTypePrompt: "Type {{word}} to confirm:",
    deleteConfirmWord: "DELETE",
    deletePasswordPrompt: "Confirm with your password:",
    deleteReauthGoogle: "Confirm identity with your Google account",
    deleteBtn: "Delete account forever",
    deleteRunning: "Deleting data...",
    deleteError: "Deletion failed. Please sign in again and retry.",
    deleteWrongPassword: "Wrong password.",
    showFullName: "Show my full name",
    showFullNameDesc: "Disable to use only initials (e.g. Jan K.)",
    showNickname: "Show my Nickname",
    showClub: "Show my Club",
    showRegion: "Show my Region",
    nicknameLabel: "Nickname / Alias (Optional)",
    nicknamePlaceholder: "e.g. Robin Hood",
    wizard: {
      btnStart: "AI Setup Wizard",
      btnDesc: "Start step-by-step profile setup.",
      titlePersonal: "Personal Info",
      firstName: "First Name",
      lastName: "Last Name",
      team: "Your Team",
      country: "Country / Federation",
      clubCity: "Club City",
      clubName: "Club Name",
      genderM: "Male",
      genderF: "Female",
      birthDate: "Date of birth",
      height: "Height (cm)",
      yearsShooting: "Years shooting",
      lessThanYear: "< 1 year",
      prehistoric: "Prehistoric archery 🦣",
      yearsSuffix: "years",
      highestLevel: "Highest competition level",
      logout: "Log out from profile",
      step1Title: "Let's set up your profile",
      step1Desc: "Hi! I'm your virtual AI Coach. To analyze your performance accurately, we need to set up your profile first. Please tell me what to call you.",
      step2Title: "Hello, {{name}}!",
      step2Desc: "Let's move on to your date of birth and gender. This is crucial for me to automatically assign you to the correct age category and match target face sizes.",
      step3Title: "Physical traits",
      step3Desc: "Your height and dominant hand give me a broader view of your technique.",
      step4Title: "Level & Equipment",
      step4Desc: "Almost done. To provide meaningful advice, I need to know what you shoot with.",
      bowType: "Bow Type",
      step5Title: "Your Sight Marks",
      step5Desc: "I've prepared a list of distances. It's worth saving your sight marks here.",
      step5Note: "Missing distances can be added at any time later in Settings.",
      distTarget: "Distance / Face",
      ext: "Extension",
      ud: "Elevation",
      lr: "Windage",
      step6Title: "Next Tournament",
      step6Desc: "You can add your upcoming competition to the schedule now.",
      calendarTitle: "Schedule",
      tournamentTab: "Tournament",
      otherTab: "Other",
      tourNamePlaceholder: "Tournament name (e.g. National Cup)",
      cityPlaceholder: "City / Address",
      notesPlaceholder: "Notes...",
      step7Title: "Archery Errands",
      step7Desc: "You can enter other events here, like a trip to the archery shop.",
      privateNamePlaceholder: "e.g. Trip to the archery shop",
      shopPlaceholder: "Archery shop / Address",
      buyPlaceholder: "What to buy?...",
      btnBack: "Back",
      btnNext: "Next",
      btnSkip: "Skip",
      btnFinish: "Finish",
      btnSaveFinish: "Save & Finish",
      assistantTitle: "Setup Assistant — {{steps}} steps",
      motivationBanner: "⚡ The more you complete, the more you gain",
      welcomeBenefit0: "AI Coach will select targets and distances for you",
      benefit1: "Statistics and charts will be accurate from the first session",
      welcomeBenefit2: "Player profile visible in club rankings",
      benefit2: "Sight settings always at hand",
      laterNote: "You can fill in missing data later in Settings, but we encourage you to do it now — it only takes a moment.",
      completionNote: "Carefully completing the assistant is the key to full GROT-X functionality — don't skip any step!",
      welcomeTitle1: "Welcome,",
      welcomeTitle2: "to",
      welcomeBtn: "GOT IT, LET'S START",
      gateNameLabel: "What should we call you?",
      gateNamePlaceholder: "Your name or nickname",
      gateNameHint: "Your name creates your profile — enter it to continue.",
      gateContinueSetup: "Continue setup",
      gateContinueNote: "Highly recommended — takes 2 minutes, but you can finish later in Settings.",
      gateStartTraining: "Start training",
      gateGoHome: "Go to home screen"
    },
    sight: {
      distTarget: "Distance / Face",
      ext: "Extension",
      ud: "Elevation",
      lr: "Windage",
      target: "Face:",
      addTitle: "Add custom distance",
      meters: "Metres",
      labelPh: "Label (optional)",
      labelHint: "e.g. blowgun, barebow — up to 10 characters",
      add: "Add",
      cancel: "Cancel",
      renameTitle: "Distance label",
      deleteAsk: "Remove this distance from the list?",
      deleteKeepsHistory: "Saved sessions stay in your statistics.",
      errDuplicate: "You already have this distance.",
      errRange: "Enter {{min}} to {{max}} metres.",
      errLimit: "Limit of {{max}} custom distances reached.",
      ownDistances: "Custom distances",
      proForMore: "PRO = 15 custom",
      setup: "Setup:",
      allSetups: "Shared",
      setupHint: "Color = setup. A shared distance shows up with every setup of its discipline."
    },
    arrows: {
      model: "Arrow Model",
      modelPh: "",
      spine: "Spine",
      spinePh: "e.g. 450",
      length: "Shaft length (inches)",
      lengthPh: 'e.g. 29.25"',
      point: "Point weight (grains)",
      pointPh: "e.g. 120 gr",
      vanes: "Vanes / Fletching",
      vanesPh: ""
    },
    bow: {
      drawWeight: "Draw weight (lbs)",
      riser: "Riser",
      riserPh: "",
      limbs: "Limbs",
      limbsPh: "",
      stabilizers: "Stabilizers",
      stabilizersPh: "",
      sight: "Sight",
      sightPh: ""
    },
    pro: {
      title: "GROT-X PRO",
      desc: "Reach the next level of precision.",
      active: "PRO ACCOUNT ACTIVE",
      inactive: "BASIC ACCOUNT",
      benefit1Title: "Unlimited History",
      benefit1Desc: "Access to all training sessions from the entire season.",
      benefit2Title: "Full Analytics",
      benefit2Desc: "Advanced shot grouping charts and form trends.",
      benefit3Title: "Personal AI Coach",
      benefit3Desc: "Intelligent tips tailored to your shot grouping.",
      benefit4Title: "Multiplayer Modes",
      benefit4Desc: "Unlimited battles in the Arena and rankings.",
      resetAI: "Reset AI greetings",
      activeAlert: "GROT-X PRO ACTIVE! ◆",
      deactiveAlert: "Free version restored.",
      resetAlert: "Reset successful!",
      trialActive: "Free PRO trial",
      trialDaysLeft: "days remaining",
      trialStatus: "PRO TRIAL ACTIVE",
      giftActive: "PRO gift",
      giftUntil: "Active until {{date}}",
      giftStatus: "PRO ACTIVE"
    },
    shareProfile: "Share Profile",
    coach: {
      title: "Coach Module",
      wip: "WORK IN PROGRESS",
      desc: "Scan your archers' QR codes and gain full control over their scores.",
      btn: "Coming soon",
      myTeam: "My coaches",
      commandCenter: "Command Center",
      slots: "Slots:",
      confirmRevoke: "Revoke this coach?",
      confirmRevokeDesc: "This operation cannot be undone.",
      revokeError: "Failed to revoke coach.",
      revokeDone: "Coach disconnected.",
      shareDesc: "Show your QR code to your Coach or copy your unique ID to give them permanent access to your training analysis.",
      shareBtn: "Share My Profile",
      activeFor: "Access active for:",
      revokeBtn: "Revoke Access",
      noCoach: "You are not sharing data with any coach.",
      panelDesc: "Go to the dedicated panel to scan students and analyze their results live.",
      openPanel: "Open Coach Panel",
      becomeTitle: "Become a GROT-X Coach",
      becomeDesc: "Coach {{count}} archer for free: scores, stats, equipment and messages. You can switch coach mode off at any time.",
      enableBtn: "Enable coach mode",
      guestBlocked: "Coach mode requires a full account.",
      enabling: "Enabling...",
      modeError: "That didn't work. Please try again.",
      moreSlotsTitle: "More slots",
      moreSlotsDesc: "Coaching more archers? Tell us how many — for now we unlock club coach packages manually.",
      disableBtn: "Disable coach mode",
      disableBlocked: "Remove all your students first to disable.",
      disableConfirmTitle: "Disable coach mode?",
      disableConfirmHint: "To confirm, tap the 0.",
      disableWrongDigit: "That's the 8 — tap the 0.",
      becomeStudentsLabel: "How many students do you plan to have?",
      becomeStudentsPlaceholder: "e.g. 10",
      becomeRequestBtn: "Send request to admin",
      becomeSending: "Sending...",
      becomeSent: "Request sent!",
      becomeSentDesc: "The administrator will review your request shortly.",
      becomeAlreadySent: "Request already sent — waiting for admin response.",
      becomeError: "Sending failed. Please try again."
    },
    tournament: {
      title: "Tournament Module",
      wip: "WORK IN PROGRESS",
      desc: "Organise your own tournament, club championship, or archery league.",
      btn: "Coming soon"
    },
    lists: {
      countries: ["Germany (DSB/WA)", "Poland (PZŁucz/WA)", "USA (USA Archery)", "Other (World Archery)"],
      eventTypes: ["WA 70m / 50m (Outdoor)", "WA 18m (Indoor)", "Field 24 targets", "3D Archery", "Local Scored Training"],
      compLevels: ["Training only (Recreation)", "Unofficial / Local tournaments", "Club Championship", "County Championship", "Regional Championship", "State Championship", "National Championship", "International (WA)"]
    }
  },
  sessionSetup: {
    arrowCount: "How many arrows were shot?",
    techNotes: "Technical notes",
    techSubtitle: "No target face",
    arrowCounter: "Arrow counter",
    saveCounter: "Save",
    notePlaceholder: "e.g. Work on bow stabilization in the left hand...",
    saveBtn: "Confirm & Save",
    trainingTopics: "Training topics",
    topicsSelected: "selected",
    topicCat_fundamenty: "Fundamentals & Posture",
    topicCat_naciag: "Setup & Draw",
    topicCat_celowanie: "Aiming & Expansion",
    topicCat_zwolnienie: "Release & Follow-through",
    topicCat_taktyka: "Tactics & Equipment",
    topicCat_wlasne: "Own topics",
    customTopicPlaceholder: "New custom topic…",
    customTopicAdd: "Add",
    customTopicRemove: "Remove from list",
    topic_fusstellung: "Stance",
    topic_tform: "Body alignment",
    topic_gewicht: "Weight distribution",
    topic_kopf: "Head position",
    topic_mindset: "Mindset",
    topic_voranschlag: "Pre-draw tension",
    topic_bogengriff: "Bow grip",
    topic_bogenarm: "Bow arm position",
    topic_ellenbogen: "Elbow rotation",
    topic_hook: "Finger placement",
    topic_auszug: "Draw",
    topic_schulter: "Shoulder down",
    topic_anker: "Anchor point",
    topic_sehne: "String alignment",
    topic_zielen: "Aiming",
    topic_rucken: "Back tension",
    topic_expansion: "Expansion",
    topic_klicker: "Clicker control",
    topic_atem: "Breathing rhythm",
    topic_visual: "Visualization",
    topic_losen: "Release",
    topic_panik: "Target panic",
    topic_nachhalten: "Follow-through",
    topic_rhythmus: "Shot rhythm",
    topic_visier: "Sight adjustment",
    topic_wind: "Wind shooting",
    topic_zeit: "Time management",
    topic_psycho: "Handling pressure",
    topic_routine: "Shot routine",
    topic_bogeneinstellung: "Bow tuning",
    topic_pfeilabstimmung: "Arrow tuning",
    topic_stabilisator: "Stabilizer setup",
    topic_nockpunkt: "Nocking point"
  },
  stats: {
    partialBadge: "Incomplete · {{label}} ends",
    title: "Stats",
    tabDaily: "Daily",
    tabRecords: "Records",
    tabPro: "PRO Progress",
    records: {
      allDistances: "All",
      lifetimeTitle: "Personal bests",
      bestTournament: "Best tournament",
      bestTraining: "Best training",
      bestAvg: "Best average",
      mostX: "Most X",
      perArrow: "pts/arrow",
      historyTitle: "Tournament history",
      starts: "Starts",
      seriesBest: "Best:",
      noData: "No results recorded",
      noTournaments: "No tournament starts recorded",
      freeLimitTitle: "You see the last year",
      freeLimitDesc: "PRO compares starts up to 5 years back",
      save: "Save",
      saving: "Saving...",
      rename: {
        title: "Fix the tournament name",
        hint: "The change applies to every start in this series, including other distances.",
        affected: "Starts to update:",
        mergeWarning: "This name already belongs to the series \"{{name}}\" — the two will be merged into one."
      },
      form: {
        openBtn: "Add a past start",
        badge: "Historical start",
        title: "Add a past start",
        namePlaceholder: "Tournament name",
        dateLabel: "Date of the start",
        scoreLabel: "Score",
        arrowsLabel: "Arrows shot",
        countersNote: "This entry goes to history and records only. It does not count towards your arrow counter or this month's stats.",
        errorDateRange: "Date is outside the available history range.",
        errorScoreTooHigh: "Score is higher than the maximum for that number of arrows.",
        errorSave: "Could not save. Check your connection."
      }
    },
    loading: "Loading...",
    noSessions: "No training sessions on this day",
    unlockDetails: "PRO Historical Details",
    unlockAnalysis: "Target spread, biomechanics & AI in PRO",
    unlockBtn: "Unlock",
    notePlaceholder: "Describe findings, paste video link (max 250 chars)...",
    invalidUrl: "Invalid URL",
    shareWithCoach: "Share with coach",
    noNote: "No note. Remember to leave findings.",
    coachTip: "Coach Tip",
    deleteSession: "Delete this session",
    deleteAnalyzing: "AI Coach is analyzing your decision...",
    deleteWarning: "Remember: mastery is built on mistakes. Every poor result is information, not a reason for shame. The AI Coach needs this record to detect patterns and help you improve your form.",
    deleteCancel: "You're right. I'll keep this result.",
    deleteConfirm: "Yes, delete this session anyway.",
    noTechSessions: "No technical training sessions saved.",
    yourNotes: "Your Notes",
    noteEditsLeft: "({{count}} edits)",
    studentFindings: "Student's Findings",
    sharedWithCoach: "Shared with coach",
    private: "Private",
    techSessionType: "Off-target session",
    techSessionTitle: "Technical Training",
    sessionFocus: "Focus of this session",
    techHistory: "Technique History",
    techHistoryDesc: "Last 3 off-target sessions",
    noSessionNotes: "No notes for this session.",
    sessionInfo: {
      solo: "Training Session",
      tournament: "Tournament",
      arena: "Arena Live",
      typeSolo: "Solo Training",
      defaultTournament: "Competition",
      dynamic: "Dynamic",
      worldBattle: "WORLD BATTLE",
      worldWin: "🏆 Win",
      worldLoss: "❌ Loss",
      worldTabWin: "🌍 WIN",
      worldTabLoss: "🌍 LOSS",
    },
    saveNote: "Save",
    cards: {
      score: "Score",
      average: "Average",
      dailyArrows: "Day Arrows",
      hitStats: "Hit Statistics",
      total: "TOTAL",
      biomechanics: "Biomechanics & Shot Grouping",
      tendency: "Grouping direction",
      error: "Dominant error",
      historyRecord: "End log",
      tenSum: "Sum 10s",
      nineSum: "Sum 9s",
      detailTable: "Detailed round table",
      durchgang: "Round",
      endLabel: "End",
      sumLabel: "Sum",
      bio: {
        coachFirst: "Unless your coach sees it differently — here is our tip for you:",
        whatToDo: "What to check",
        lastTrainings: "last {{count}} trainings",
        handPromptTitle: "Set your handedness to personalise the tips",
        handPrompt: "Left/right tips depend on whether you shoot right- or left-handed. Set this in your profile and we'll tailor the advice to you.",
        handCta: "Set handedness in profile",
        adjustedRH: "Tips adjusted for: right-handed",
        adjustedLH: "Tips adjusted for: left-handed",
        tendencyTitle: "Grouping direction",
        tendencyAbout: "Shows where the centre of your group sits on average relative to the centre of the target. This is a steady, systematic offset. Adjust your sight first — if the group is tight but shifted, it is usually just a sight setting. If the group keeps drifting the same way despite a correct sight, the cause is in your technique.",
        tendCenter: "Your group centre is on target — aiming and sight are set correctly. Now focus on group tightness (see \"Dominant error\").",
        tendLeft: "Arrows land left. Common causes: string picture sitting to the right at aim, grip torque, or a collapsing / overly \"defensive\" bow arm. Check: neutral grip pressure without twisting, actively drive the bow arm into the target, keep the same string picture against the riser every shot.",
        tendRight: "Arrows land right. Common causes: plucking the string on release, canting the bow to the right, or string picture sitting to the left. Check: a clean rearward release along the neck (elbow back, not out), a vertical bow, a consistent string picture.",
        tendLeftLH: "Arrows land left. Common causes: plucking the string on release, canting the bow to the left, or string picture sitting to the right. Check: a clean rearward release along the neck (elbow back, not out), a vertical bow, a consistent string picture.",
        tendRightLH: "Arrows land right. Common causes: string picture sitting to the left at aim, grip torque, or a collapsing / overly \"defensive\" bow arm. Check: neutral grip pressure without twisting, actively drive the bow arm into the target, keep the same string picture against the riser every shot.",
        tendUp: "Arrows land high. Common causes: too much pressure on the lower grip, more index-finger curl, a longer draw, or an open jaw instead of teeth together. Check: consistent, centred grip pressure and a repeatable anchor.",
        tendDown: "Arrows land low. Common causes: draw-length collapse, a dropping bow arm, or string contact with clothing or chest. Check: hold back tension to the end, strong follow-through, remove string contact with clothing.",
        errorTitle: "Dominant error",
        errorAbout: "Shows the shape of your spread — which way your group stretches the most. This cannot be fixed with the sight; it is about technical consistency shot after shot.",
        errSymm: "Spread is even horizontally and vertically — no dominant error axis, good technical balance. Work on shrinking the group overall: the same rhythm and timing each shot, plus fitness.",
        errHoriz: "Mostly horizontal spread (left–right). The prime suspect is string alignment — your \"rear sight\". An inconsistent string position against the riser produces horizontal spread. Add grip torque, plucking, and bow cant. Settle on ONE consistent string picture and repeat it every shot; neutral grip; a clean rearward release.",
        errVert: "Mostly vertical spread (up–down). Prime suspects: inconsistent anchor, varying finger pressure on the string, release quality, draw-length collapse, a collapsing bow arm. Vertical spread grows especially at longer distances (arched trajectory). Work on a steady anchor and finger pressure, continuous back tension (clicker), and a full follow-through."
      }
    },
    pro: {
      loading: "Analyzing historical data...",
      noData: "No data to analyze",
      last12weeks: "Stats from last 12 weeks",
      estScore720: "Estimated Score (720)",
      heatmapTitle: "Scatter Heatmap",
      decline: "Decline",
      noDistanceData: "No data for the selected distance.",
      lastN: "Last",
      title: "PRO Long-term Analysis",
      desc: "Unlock advanced progress charts, Personal Bests (PB), hit zone distribution, and trends for every distance.",
      btnUnlock: "Unlock Access",
      pb: "Personal Best (PB)",
      fromArrows: "From {{count}} arrows",
      estTournament: "Estimated Tournament (72)",
      formTrend: "Form Trend (last 3)",
      unitPtsArrow: "pts/arrow",
      zonesTitle: "Hit zones (All season)",
      progressCurve: "Progress Curve (Average)",
      overviewTitle: "Overview",
      trainingsCount: "{{count}} TRAINING SESSIONS",
      totalDistArrows: "Total at this distance: {{count}} arrows",
      unlockDesc: "Unlock scatter map, form charts, endurance index and full data.",
      allHistory: "Full history",
      heatmapFrom: "Visualization from",
      sessions: "sessions",
      training: "Session",
      openSession: "View session",
      arrowsCount: "arrows",
      enduranceTitle: "Endurance Index",
      enduranceSub: "From last {{count}} sessions",
      firstHalf: "1st Half (Avg.)",
      secondHalf: "2nd Half (Avg.)",
      weeklyArrows: "Arrow Count (Weekly)",
      weeklyArrowsShort: "Arrows / week (12 wk.)",
      weeklyPointsShort: "Ring avg / week (12 wk.)",
      sessionListTitle: "Session list",
      allDistances: "All distances",
      distAnalysisTitle: "Distance analysis",
      distAnalysisSub: "Data below reflects your selection",
      distLabel: "Distance",
      typeLabel: "Session type",
      tooFewSessions: "Too little data. Save more training sessions.",
      arrows: "Arrows",
      zones: {
        gold: "Gold",
        red: "Red",
        blue: "Blue",
        black: "Black",
        white: "White",
        miss: "Misses",
        center: "Centre",
        right: "To the right",
        left: "To the left",
        up: "Upwards",
        down: "Downwards",
        symm: "Symmetric",
        horiz: "Horizontal (Grip / Bowstring)",
        vert: "Vertical (Posture / Aiming)"
      },
      chart: {
        tooFewData: "Too little data to generate progress curve (min. 2 training sessions)",
        pts: "pts"
      },
      sight: {
        ok: "Sight set correctly",
        adjustRight: "Sight → right",
        adjustLeft: "Sight → left",
        adjustUp: "Sight → up",
        adjustDown: "Sight → down",
        slightly: "slightly",
        clearly: "clearly",
        significantly: "significantly",
        infoTitle: "Sight adjustment tip",
        infoBody: "If your coach doesn't have a personal recommendation — here's our tip to improve accuracy with your sight settings:",
        infoRule: "\"Follow your arrows\" rule: move the sight in the direction your arrows land. Arrows right = sight right. Arrows up = sight up.",
        infoNote: "Approximate values with ±20% accuracy. Depend on equipment setup and vary between sights.",
        yourCorrection: "Your correction:",
        onSight: "on sight",
        approxNote: "* Approximate estimate (±20%). Exact value depends on your equipment settings.",
        now: "NOW"
      }
    },
    zoom: {
      title: "Shot Grouping Analysis",
      all: "ALL",
      heatmap: "Heatmap"
    }
  },
  coachDashboard: {
    newStudentBadge: "New",
    inviteSentTitle: "Invitation sent",
    inviteSentDesc: "{{name}} has received the invitation. They will appear in your student list once they accept it on their phone.",
    inviteSentDescNoName: "The student has received the invitation. They will appear in your list once they accept it.",
    inviteSentOk: "OK",
    newStudentDot: "New student ({{count}})",
    title: "Squad",
    subtitle: "Coach Panel",
    addBtn: "Add",
    allGroups: "All",
    addStudentHeader: "Add student",
    idPlaceholder: "Paste student's unique ID...",
    cameraError: "Camera unavailable.",
    cameraErrorDesc: "Enter student ID manually above",
    orScanQr: "Or scan QR",
    journalTitle: "Group Journal",
    journalCount: "{{count}}/5 entries",
    journalPlaceholder: "Training plan, notes or YouTube link...",
    journalAddBtn: "Add Entry",
    allStudents: "All Students",
    groupList: "Group List",
    noStudents: "No students in this view",
    newTraining: "New training",
    manageGroups: "Manage Groups",
    removeStudent: "Remove student",
    selected: "Selected",
    writeBtn: "Write",
    newGroupTitle: "New Group",
    newGroupPlaceholder: "E.g. Beginners, Squad...",
    cancel: "Cancel",
    save: "Save",
    manageGroupsTitle: "Manage Groups",
    manageGroupsDesc: "Choose which groups this student belongs to",
    done: "Done",
    limitTitle: "Limit Reached",
    limitDesc: "You already have 5 entries in this group. Adding a new one will remove the oldest. Its content is:",
    replaceOld: "Replace Old",
    messageTitle: "Message",
    recipients: "Recipients:",
    recipientsCount: "{{count}} students",
    titlePlaceholder: "Notification title",
    contentPlaceholder: "Message content...",
    processing: "Processing...",
    sendTo: "Send to {{count}} students",
    deleteTitle: "Remove student?",
    deleteDescPre: "To confirm removal, press the number",
    deleteDescPost: ". This prevents accidental deletion in your pocket.",
    cancelOp: "Cancel operation",
    deleteNoteTitle: "Delete this entry?",
    deleteNoteDesc: "This action cannot be undone.",
    confirm: "Confirm",
    newestEntry: "Latest",
    journalPrivate: "Entries visible only to you (coach)",
    editGroupTitle: "Rename group",
    deleteGroupTitle: "Delete group?",
    deleteGroupDesc: "Deleting the group will also remove all its entries and student assignments. This cannot be undone.",
    deleteGroupBtn: "Delete group",
    editGroupBtn: "Rename",
    toastSelf: "You cannot add yourself!",
    toastLimit: "Student limit reached ({{limit}}). Upgrade your plan.",
    toastAlready: "This student is already on your list.",
    toastNotFound: "Student not found. Please verify the ID.",
    toastAdded: "Student added!",
    toastInviteSent: "Invitation sent. The student must accept it.",
    toastError: "System error occurred.",
    toastRemoved: "Student removed from squad and groups.",
    toastFillFields: "Please fill in the title and message content!",
    toastMsgSent: "Messages sent to students' devices!",
    toastConnError: "Connection error. Please try again.",
    toastGroupCreated: "Group created!",
    toastGroupError: "Error creating group.",
    toastGroupRenamed: "Group renamed!",
    toastGroupDeleted: "Group deleted.",
    toastSaveError: "Save error.",
    toastNoteAdded: "Entry added to journal!",
    toastNoteError: "Error adding entry.",
    noTrainings: "No trainings",
    today: "Today",
    yesterday: "Yesterday",
    daysAgo: "{{count}} days ago",
    selectAll: "Select visible",
    deselectAll: "Deselect visible",
    defaultStudentName: "Student",
    viewGroups: "Groups",
    viewStudents: "Students",
    allStudentsCard: "All students",
    studentsCount: "{{count}} students",
    noGroups: "No groups",
    createFirstGroup: "Create group",
    upcomingTitle: "Upcoming sessions",
    upcomingNone: "No sessions",
    upcomingOpenCalendar: "Open calendar",
    newEventBtn: "New session",
    studentTournamentsTitle: "Student competitions",
    selectMode: "Select",
    sendMessageBtn: "Send message",
    newEntryBtn: "New entry",
    previousEntries: "Previous entries",
    journalLimitHint: "Max. 5 entries – a sixth replaces the oldest (we'll ask first).",
    pickGroupHint: "Pick a group",
    newTrainingsTitle: "New sessions",
    formTitle: "Form",
    formNowPrev: "now {{now}} · last {{prev}}",
    inactiveShowOnList: "Show in student list",
    inactiveShowAll: "Show all ({{count}})",
    inactiveFilterChip: "No training {{days}}+ days",
    lastSessionScore: "{{score}} pts / {{arrows}} arrows",
    lastSessionArrows: "{{count}} arrows",
    lastSessionTech: "Technique",
    trendUp: "Form improving",
    trendDown: "Form declining",
    pauseBadge: "Paused",
    pauseReminder: "Paused for a month — check in",
    pauseKeepShort: "Keep",
    pauseHintShort: "Pause (e.g. illness): hidden here, back once after a month. Their first session ends it.",
  },
  coachSheet: {
    close: "Close",
    chat: "Messages",
    loadError: "Couldn't load sessions.",
    last14: "Last 14 days",
    legendScore: "scored",
    legendPractice: "technique",
    sessions: "Sessions",
    arrows: "Arrows",
    avgArrow: "Ø arrow {{distance}}",
    results: "Scores {{distance}}",
    lastScores: "Scores: {{list}}",
    gold: "In the gold (X/10/9): {{count}} of {{of}} arrows",
    misses: "M: {{count}}",
    round2Weaker: "Round 2 weaker by {{count}} pts on average",
    round2Stronger: "Round 2 stronger by {{count}} pts on average",
    endRange: "End spread in last session: {{min}}–{{max}} pts",
    noData: "No sessions in the last 4 weeks.",
    today: "today",
    tomorrow: "tomorrow",
    inDays: "in {{count}} days",
    studentWrote: "Student wrote · {{date}}",
    techTraining: "Technical training",
    markSeen: "Seen",
    seenDone: "Checked — you have seen this note",
    yourReply: "Your note",
    replyPlaceholder: "Reply briefly…",
    send: "Send",
    replyHint: "Saved as your note on this session.",
    saveError: "Couldn't save. Try again.",
    fullProfile: "Full student profile",
    tilesCaption: "Next to each number: change vs the 14 days before",
    lastAvg: "Ø per arrow: {{list}}",
    scoreOf: "{{score}}/{{arrows}} arr.",
    fromLast: "(last {{count}})",
    pausedSince: "Paused since {{date}} — no “no training” alerts.",
    pauseReminderText: "Paused for a month now. Keep pausing?",
    pauseKeep: "Keep pausing",
    pauseEnd: "End pause",
  },
  studentProfile: {
    focusLoadError: "Could not load: {{parts}}",
    focusPart_user: "student profile",
    focusPart_goal: "coach goals",
    focusPart_sessions: "sessions",
    focusLabel: "Focus",
    focusNone: "No focus · add a “Focus” entry in the Diary",
    focusModalTitle: "Student's focus",
    focusModalCurrentLabel: "Current focus",
    focusModalEdit: "Edit",
    focusModalNotYours: "This focus was set by the student or another coach — you can set a new one below.",
    focusModalNoTopic: "No topic — every session counts",
    focusModalTopicHint: "The start date stays — dots are recounted for the new topic.",
    focusModalGoalWarning: "The student already has {{count}} sessions — this focus will be marked as completed.",
    focusModalEnd: "End focus early",
    focusModalEndConfirm: "End this focus? The student will see no active focus until you set a new one.",
    focusModalEndConfirmBtn: "Yes, end it",
    focusModalNewTitle: "New focus",
    focusModalNewPlaceholder: "Briefly describe what the student should focus on…",
    focusModalSaveNew: "Set focus",
    focusModalReplace: "Replace focus",
    focusReplaceConfirm: "Replace the current focus “{{title}}”?",
    focusReplaceHint: "It is not finished yet — its progress ends and the new focus starts counting from zero.",
    focusReplaceYes: "Yes, replace",
    loading: "Loading student profile...",
    headerLabel: "Student Profile",
    arrowsMonth: "Arrows/Mo.",
    avg14days: "Avg. 14 days",
    lastScore: "Last Score",
    nextGoal: "Next Goal (Tournament)",
    statsScore: "Score",
    statsAvg: "Average",
    statsInnerX: "Inner X",
    studentNoteLabel: "Student feelings:",
    noStudentNote: "No student entry.",
    fullAnalytics: "Full Analytics",
    tabOverview: "Overview",
    tabDiary: "Diary",
    tabAnalytics: "Analytics",
    noSessions: "No training sessions yet",
    trendTitle: "Score curve",
    coachNoteLabel: "Your notes for the training of {{date}}",
    coachNoteLabelNoDate: "Your notes",
    coachNoteEdits: "({{count}} edits)",
    showDetailedStats: "Show detailed statistics",
    hideDetailedStats: "Hide statistics",
    coachNotePlaceholder: "Enter a short tip for the student...",
    coachNoteCancel: "Cancel",
    coachNoteSaving: "Saving...",
    coachNoteSave: "Save",
    coachNoteEmpty: "No notes. Student cannot see this entry.",
    privateNoteTitle: "Coach Note",
    privateNoteSubtitle: "Athlete Development Card",
    privateNoteDesc: "Information visible only to you. The student has no access.",
    privateNoteSaving: "Saving...",
    privateNoteSave: "Save Notes",
    privateNotePlaceholder: "Enter note...",
    privateNoteLabel0: "Long-term goal",
    privateNoteLabel1: "Current corrections",
    privateNoteLabel2: "Other observations",
    hardwareTitle: "Equipment Card",
    hardwareSubtitle: "Configuration & sight",
    hardwareBow: "Bow & equipment",
    hardwareBowType: "Bow type",
    hardwareDraw: "Draw weight",
    hardwareRiser: "Riser",
    hardwareLimbs: "Limbs",
    hardwareStabilizers: "Stabilisers",
    hardwareSight: "Sight",
    hardwareSightMarks: "Sight marks settings",
    hardwareSightExt: "Extension (Ext)",
    hardwareSightUD: "Vertical (U/D)",
    hardwareSightLR: "Horizontal (L/R)",
    hardwareNoSight: "Student has not set up sight marks yet",
    hardwareEmpty: "Student has not entered any equipment yet",
    hardwareActive: "Active",
    drawLengthShort: "Draw",
    sightExtShort: "Ext",
    sightUDShort: "U/D",
    sightLRShort: "L/R",
    historyLabel: "History ({{index}}/{{count}})",
    tournamentsTitle: "Schedule",
    tournamentsSubtitle: "Student's upcoming tournaments",
    noLocation: "No location",
    today: "Today",
    tomorrow: "Tomorrow",
    inDays: "In {{count}} days",
    techSectionsTitle: "Technical Training",
  },
  coachInvite: {
    title: "Coach invitation",
    message: "{{name}} wants to follow you as a coach.",
    desc: "By accepting, you share your first and last name, your club and all your shooting sessions and statistics with the coach. The coach cannot see your date of birth or e-mail address. You can revoke this access anytime in Settings.",
    accept: "Accept",
    reject: "Reject",
    unknownCoach: "Coach",
  },
  battleInvite: {
    title: "Battle invitation",
    message: "{{name}} invites you to a battle.",
    accept: "Accept",
    reject: "Decline",
    unknownHost: "Archer",
  },
  delayMirror: {
    techDiscard: "Exit without training",
    techDiscardHint: "The arrows still stay in the daily arrow counter.",
    techSynced: "Saved — safe to exit",
    techSyncPending: "Saved on phone — will upload when online",
    techSyncError: "Couldn't save arrows",
    pauseCountTitle: "How many arrows did you shoot?",
    pauseCountSkip: "Skip",
    techToggle: "Technical training",
    techArrowsDone: "You shot {{count}} arrows during Delay Mirror training",
    techArrowsDone_one: "You shot {{count}} arrow during Delay Mirror training",
    techArrowsSoFar: "Arrows this session",
    techDone: "Done",
    techSummaryTitle: "Save technical training?",
    techSaveError: "Saving failed. Please try again.",
    title: "Delay Mirror",
    description: "Live camera with {{seconds}}s delay — watch your own shot technique right after returning from the target.",
    proRequired: "Feature available in GROT-X PRO",
    feature1: "{{seconds}}s delay — see yourself \"after the shots\"",
    feature2: "Front camera — set up your phone and shoot",
    feature3: "\"Walk to target\" pause — saves battery",
    feature4: "Zero upload — video stays in RAM only",
    livePreviewBtn: "Show live preview only",
    livePreviewInfo: "FREE mode — camera preview only. Unlock 15s delay with PRO.",
    back: "Back",
    unsupportedTitle: "Browser not supported",
    unsupportedDesc: "Delay Mirror requires Chrome on Android or Safari on iOS 15+.",
    errorTitle: "Error",
    permissionDenied: "Camera permission denied.",
    cameraError: "Camera error",
    idleDescription: "Camera records live. When you return from the target you'll see your shot from {{seconds}} seconds ago.",
    delayBadge: "{{seconds}}s delay · front camera",
    delayLabel: "Delay",
    grid: "Grid",
    modeLabel: "Session mode",
    modeMirror: "Mirror only",
    modeRecord: "Mirror + recording",
    modeMirrorHint: "Nothing is saved — the image runs delayed and is discarded.",
    modeRecordHint: "A clip builds up in phone memory; you can watch it when you finish.",
    recOn: "Recording",
    recOff: "Record",
    pauseBtn: "Pause",
    resumePause: "I'm back",
    pausedBanner: "Paused",
    drainHint: "{{seconds}}s left from the buffer",
    seriesTargetTitle: "Which target did you shoot?",
    seriesTargetSpot: "Spot",
    seriesTargetRound: "Round",
    seriesEnterTitle: "Enter arrows",
    seriesArrowsTitle: "Arrows per end?",
    seriesGoCollect: "Mark arrows",
    seriesGoCollectHint: "Take the phone with you to the target — you'll tap each arrow where it sits.",
    seriesWatchOnly: "Skip analysis",
    passReviewTitle: "This end",
    passReviewNext: "Next end",
    passReviewNoShots: "No arrows marked",
    markShots: "Mark arrows",
    markRedo: "Mark again",
    markShotN: "Arrow {{n}}",
    markUndo: "Undo",
    markFinish: "Done",
    exportWithTarget: "Clip with target",
    exportRunning: "Building clip",
    exportHint: "The clip plays through once — it takes as long as the recording.",
    exportCancel: "Cancel",
    exportFailed: "Did not work — share the clip without the target.",
    exportDone: "Clip ready",
    watchAgain: "Play",
    watchFullscreen: "Fullscreen",
    exportBack: "Back to replay",
    ffHoldHint: "Hold: 4x, release: normal speed",
    noteInputPlaceholder: "Note for shot {{n}}",
    noteDoneHint: "Done — resume playback",
    notesPro: "Shot notes — PRO",
    markHint: "Play the clip and tap at each shot",
    markDone: "{{done}} of {{total}} marked",
    expertMode: "Expert mode",
    simpleHint: "{{seconds}}s delay, landscape, nothing is saved.",
    delayReset: "Back to {{seconds}}s",
    endSeries: "End of end",
    endSeriesSub: "analysis",
    seriesEnterHint: "Where did arrow {{n}} land?",
    seriesEnterDone: "All arrows entered",
    seriesUndo: "Undo",
    seriesToReplay: "Watch and mark",
    seriesClock: "{{score}} at {{clock}}",
    seriesCenter: "{{score}} in the centre",
    seriesClipWarning: "The clip only lives until the session ends — share it before you finish. The shot placement itself is saved for good.",
    finish: "Finish",
    finishToReplay: "replay",
    finishToMenu: "to menu",
    delaySliderHint: "Set from {{min}}s to {{max}}s",
    rotateHint: "Rotate phone to landscape for a better view",
    chooseOrientation: "Choose recording orientation",
    orientationPortrait: "Portrait",
    orientationLandscape: "Landscape",
    privacyNote: "Privacy: your recording never leaves your device. This whole feature runs locally — nothing is sent to any server.",
    start: "Start",
    stop: "Stop",
    buffering: "Buffering…",
    bufferingHint: "Wait {{seconds}}s for the delayed image to appear",
    pauseTitle: "Paused",
    pauseHint: "Camera off · saving battery",
    resumeBtn: "I'm back",
    endSession: "End session",
    shareIdle: "Share video",
    savePro: "Save video — PRO",
    shareSharing: "Sharing…",
    shareProcessing: "Preparing video…",
    shareSaved: "Saved to disk",
    shareError: "Error — try again",
    afterShots: "Walk to target",
    pauseTooltip: "Pause",
    replayTitle: "Replay session",
    replaySpeed: "Speed",
    replayBack5: "-5s",
    replayFwd5: "+5s",
    replayRestart: "Restart",
    toLandscape: "Landscape mode",
    toPortrait: "Portrait mode",
    positioningTitle: "Position your device",
    positioningHint: "Place the phone at shoulder height, an arm's length from the shooting line. When you're ready — start.",
    positioningStart: "Start recording",
    positioningStartMirror: "Start mirror",
    setupInstructionsTitle: "Device Setup Instructions",
    setupInstructions: "1. HEIGHT\nPosition your device so the camera lens is exactly at your shoulder height. This prevents image distortion and provides the most natural view of your archery geometry.\n\n2. DISTANCE AND PLACEMENT\nPlace the device on the line where you stand, at arm's length (measured to fingertips).\n\n3. OBSERVATION POSITION\nChoose the angle based on what you want to analyze:\n\nBehind you: to monitor scapula movement and elbow line.\n\nFrom chest side: to monitor release hand movement and release moment.\n\n4. TIP\nRemember to position the device perpendicular to you — this will help you avoid errors in posture assessment.",
    setupInstructionsClose: "Close",
  },
};
